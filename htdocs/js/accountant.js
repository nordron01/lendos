$(document).ready(function() {
    ...
    socket.emit('accountant_load_page',{id:'1'});
  	socket.emit('sklad_load_data',{id:'id'});
    socket.emit('desktop_order_load_page',{id:'id'});
    socket.emit('order_position_load_data',{id:'id'});
    ...
    */
    ...
    var DateFieldTobaccoLoad = function(config) {
        jsGrid.Field.call(this, config);
    };

    DateFieldTobaccoLoad.prototype = new jsGrid.Field({
        sorter: function(date1, date2) {
            return new Date(date1) - new Date(date2);
        },
        itemTemplate: function(value) {
...
$('.shorting').keyup(function() {
    ...
    socket.on('bd_date_load_tobacco', function(data) {
        //console.log(data);
        var db_load = {
            ...
            loadData: function(filter) {
                ...
                return $.grep(this.clients, function(client) {
                    ...
                    return (filter.idtobacco === undefined || client.idtobacco === filter.idtobacco) &&
                        (filter.price == undefined || client.price == filter.price) &&
                        (filter.sales == undefined || client.sales == filter.sales) &&
                        (filter.lukas === undefined || client.lukas === filter.lukas) &&
                        (filter.dislukas === undefined || client.dislukas === filter.dislukas) &&
                        (filter.col === undefined || client.col === filter.col) &&
                       ...
                       // (!filter.dates.to || new Date(client.dates) <= filter.dates.to) &&
                        ...
                        (!filter.taste || client.taste.indexOf(filter.taste) > -1) &&
                        (!filter.photo || client.photo.indexOf(filter.photo) > -1) &&
                        (!filter.strong || client.strong.indexOf(filter.strong) > -1) 
            ...
            insertItem: function(insertingClient) {
                ...
                socket.emit('accountant_insert_tobacco', insertingClient);
            },
            updateItem: function(updatingClient) {
                socket.emit('accountant_update_tobacco', updatingClient);
            },
            deleteItem: function(deletingClient) {
                socket.emit('accountant_delete_tobacco', deletingClient);
        ...
        console.log(db_load);
        //jsGrid.fields.DateFieldTobaccoLoad = DateFieldTobaccoLoad;

        	//console.log(db_load.clients[0]);
            ...
            $("#jsGrid_Load").jsGrid({
                ...
                fields: [
                    ...
                    { name: "idtobacco", title: "Id", type: "number",  width: 30,align: "center" },
                    { name: "name", title: "Фирма", type: "text",  width: 30,align: "center" },
                    { name: "taste", title: "Вкус", type: "text",  width: 30 },
                    { name: "price", title: "Цена", type: "number", width: 30, align: "center" },
                    { name: "sales", title: "Продажа", type: "number", width: 30, align: "center" },
                    { name: "photo", title: "Фото", type: "text",  width: 30, align: "center" },
                    { name: "strong", title: "Крепость", type: "text", width: 30, align: "center" },
                    { name: "description", title: "Описание", type: "text",  width: 30, align: "center" },
                    { name: "lukas", title: "Лайк", type: "number", width: 10, align: "center" },
                    { name: "dislukas", title: "Дизлайк", type: "number", width: 10, align: "center" },
                    { type: "control", modeSwitchButton: false, editButton: true, align: "center", width: 20 } //readOnly: true
    ...
    socket.on('sklad_load_data_page', function(data) {
    	console.log('asda');
        console.log(data);
        var bd_sklad = {
            loadData: function(filter) {
            ...
            insertItem: function(insertingClient) {
                this.clients.push(insertingClient);
                socket.emit('sklad_insert_data', insertingClient);
            },
            updateItem: function(updatingClient) {
                socket.emit('sklad_update_data', updatingClient);
            },

            deleteItem: function(deletingClient) {
                socket.emit('sklad_delete_data', deletingClient);
                var clientIndex = $.inArray(deletingClient, this.clients);
                this.clients.splice(clientIndex, 1);
            }
        };

        window.bd_sklad = bd_sklad;
        bd_sklad.clients = data;
       // jsGrid.fields.DateFieldTheachers = DateFieldTheachers;
        ...
        $("#jsGrid_Sklad").jsGrid({
            ...
            controller: bd_sklad,
            fields: [
                { name: "idsklad_tovar", title: "id", type: "number", width: 40 , align: "center"},
                { name: "name_tovar", title: "Название", type: "text", width: 40, align: "center" },
                { name: "measurement", title: "Статус", type: "text", width: 40, align: "center" },
                { name: "class_tovar", title: "Статус", type: "text", width: 40, align: "center" },
                { name: "description", title: "Статус", type: "text", width: 40, align: "center" },

                { name: "quantity", title: "Статус", type: "number", width: 40, align: "center"},
                { name: "starting_price", title: "Статус", type: "number", width: 40, align: "center" },
				{ name: "finish_price", title: "Статус", type: "number", width: 40, align: "center" },

                
              //  { name: "create_date", title: "Дата создания", type: "DateFieldTheachers", width: 40, align: "center" },
              //  { name: "update_date", title: "Дата обновления", type: "DateFieldTheachers", width: 40, align: "center" },
    ...
    $(".config-panel2 input[type=checkbox]").on("click", function() {
        var $cb = $(this);
          $("#jsGrid_Sklad").jsGrid("option", $cb.attr("id"), $cb.is(":checked"));
        $("#jsGrid_Sklad").data("JSGrid").refresh();
    ...
    socket.on('bd_desktop_order_load_page', function(data) {
        console.log(data);
        var bd_desktop_order = {
            loadData: function(filter) {
                return $.grep(this.clients, function(client) {
                    return (!filter.login || client.login.indexOf(filter.login) > -1) &&
                        (!filter.firstname || client.firstname.indexOf(filter.firstname) > -1) &&
            ...
            insertItem: function(insertingClient) {
                this.clients.push(insertingClient);
                socket.emit('desktop_order_insert_tobacco', insertingClient);
            },
            updateItem: function(updatingClient) {
                socket.emit('desktop_order_update_tobacco', updatingClient);
            },

            deleteItem: function(deletingClient) {
                socket.emit('desktop_order_delete_tobacco', deletingClient);
                var clientIndex = $.inArray(deletingClient, this.clients);
                this.clients.splice(clientIndex, 1);
            }
        };

        window.bd_desktop_order = bd_desktop_order;
        bd_desktop_order.clients = data;
       jsGrid.fields.DateFieldTheachers = DateFieldTheachers;
       
        $("#jsGrid_Desktop_order").jsGrid({
            width: "100%",
            height: "640px",
            filtering: true,
            editing: true,
            ...
            pageSize: 20,
            pageButtonCount: 5,
            controller: bd_desktop_order,
            fields: [
                { name: "idorder_desktop", title: "id", type: "number", width: 40, align: "center" },
                { name: "desktop_name", title: "Название", type: "text", width: 40, align: "center" },
                { name: "status_order", title: "Статус", type: "text", width: 40, align: "center" },
    ...
    $(".config-panel3 input[type=checkbox]").on("click", function() {
        var $cb = $(this);
        $("#jsGrid_Desktop_order").jsGrid("option", $cb.attr("id"), $cb.is(":checked"));



        $("#jsGrid_Desktop_order").data("JSGrid").refresh();
    });
    ...
    socket.on('order_position_load_page', function(data) {
        ...
        var db_group_info = {
            ...
            insertItem: function(insertingClient) {
                this.clients.push(insertingClient);
                socket.emit('order_position_insert_data', insertingClient);
            ...
            updateItem: function(updatingClient) {
                ...
                socket.emit('order_position_update_data', updatingClient);
            },

            deleteItem: function(deletingClient) {
                socket.emit('order_position_delete_data', deletingClient);
                var clientIndex = $.inArray(deletingClient, this.clients);
                this.clients.splice(clientIndex, 1);
            }
        ...
        $("#jsGrid_order_position").jsGrid({
            ...
            fields: [
                { name: "idorder_position", title: "id", type: "number", width: 40, align: "center" },
                { name: "id_desktop", title: "Стол", type: "number", width: 40, align: "center" },
                { name: "id_sklad_tovar", title: "Товар", type: "number", width: 40, align: "center" },
                { name: "quantity_tovar", title: "Кол-во", type: "number", width: 40, align: "center" },
                { name: "sale", title: "Цена", type: "number", width: 40, align: "center" },
                { type: "control", modeSwitchButton: false, editButton: true }
            ]
    ...
    $(".config-panel4 input[type=checkbox]").on("click", function() {
        var $cb = $(this);
        $("#jsGrid_order_position").jsGrid("option", $cb.attr("id"), $cb.is(":checked"));
        $("#jsGrid_order_position").data("JSGrid").refresh();
    });
    ...
    function errormsg(cases) {
        ...
        switch (cases) {
            case 1:
                { srt = '<div class="close_window">x</div><p>Добавлено</p>'; }
                break;
            case 2:
                { srt = '<div class="close_window">x</div><p>Удалено</p>'; }
                break;
            case 3:
                { srt = '<div class="close_window">x</div><p>Обновлено</p>'; }
                break;
            case 4:
                { srt = '<div class="close_window">x</div><p>Добавлено</p>'; }
                break;
            case 5:
                { srt = '<div class="close_window">x</div><p>Не верный логин или пароль</p>'; }
                break;
